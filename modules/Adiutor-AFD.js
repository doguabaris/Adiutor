...
function callBack() {
	...
	let pageTitle = String( mw.config.get( 'wgPageName' ) ).replace( /_/g, ' ' );
	...
	function replaceParameter( input, parameterName, newValue ) {
		...
		if (input.indexOf( '$' + parameterName ) !== -1) {
	...
	if (addNominationToNoticeboardByFindLast) {
		...
		api.get( {
			...
			const NominatedBefore = pageContent.indexOf( '{{' + noticeBoardTitle + '/' + pageTitle.replace( /_/g, ' ' ) + '}}' ) !== -1;
			if (!NominatedBefore) {
				api.postWithToken( 'csrf', {
					action: 'edit',
					title: noticeBoardTitle,
	...
	function logNomination() {
		if (adiutorUserOptions.speedyDeletion.afdLogNominatedPages === true) {
			...
			api.get( {
				...
				if (pageContent.indexOf( sectionTitle ) !== -1) {
